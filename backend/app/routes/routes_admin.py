...
async def list_dataset_items(
    ...
    def predicate(item: dict) -> bool:
        ...
        # Filter by status
        if status:
            item_status = item.get("review_state", {}).get("status")
            if item_status != status:
                return False
        
        # Filter by finalized
        if finalized is not None:
            item_finalized = item.get("review_state", {}).get("finalized", False)
            if item_finalized != finalized:
                return False
        ...
        if search:
            ...
            if search_lower not in content_str:
                return False
    ...
    return {
        "items": [DatasetItemResponse(**item) for item in paginated_items],
        "total": total_count,
        "limit": limit,
        "offset": offset,
